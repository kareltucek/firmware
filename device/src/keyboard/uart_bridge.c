...
typedef struct {
    ...
    uint8_t* rxBuffer;
    uint8_t txBuffer[UART_MAX_SERIALIZED_MESSAGE_LENGTH];
        ...
        case UartControl_ValidMessage:
            {
                ...
                Messenger_Enqueue(connectionId, remoteDeviceId, oldPacket, len, CRC_LEN);
            }
            break;
        case UartControl_InvalidMessage: {
                uartState->invalidMessagesCounter++;
                const char *out1, *out2;
                Messenger_GetMessageDescription(uartState->rxBuffer, CRC_LEN, &out1, &out2);
...
static void initUart(
    ...
    uartState->rxBuffer = MessengerQueue_AllocateMemory();
    UartParser_SetRxBuffer(&uartState->parser, uartState->rxBuffer);
    UartParser_SetTxBuffer(&uartState->parser, uartState->txBuffer);
}
